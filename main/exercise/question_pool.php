...
if ($exerciseId > 0) {
    ...
    $sql = " (
                SELECT q.*, r.exercice_id FROM $TBL_QUESTIONS q
    ...
    if (!empty($session_id) && $session_id != '-1') {
        ...
        $sql = "SELECT DISTINCT qu.id, question, qu.type, level, q.session_id, qt.exercice_id
                FROM
                $TBL_QUESTIONS as qu,
...
//echo "<pre>";print_r($mainQuestionList);//die;
...
if (is_array($mainQuestionList)) {
    foreach ($mainQuestionList as $question) {
        ...
        $row[] = $question_type;
		//for Exercise quiz topic 
		$objExercise = new Exercise();
		$objQuizDetails = $objExercise->getQuiz($question['exercice_id']);
		if(!empty($objQuizDetails->topic)){
			$topic = $objQuizDetails->topic;
		}else
		{
			$topic = '-';
		}
		$row[] = $topic;
		//end for Exercise quiz topic 
